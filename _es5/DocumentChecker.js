...
let generateOptions = createGenerateOptions((generateOptions, previousChecks) => {
  return {
    ...
    unique: settings => generateOptions(previousChecks.concat([async (property, data, config, field, document, collection, runtimeSettings) => {
      ...
      let resultDoc = await collection.findOne({
        [field]: data
      }, {
        $projection: {
          _id: 1
    ...
    object: (settings = {}) => generateOptions(previousChecks.concat([(property, value, config, field, document, collection, runtimeSettings) => {
      ...
      if (settings.keysOf) {
        Object.keys(value).forEach(key => {
          settings.keysOf._.forEach(checker => checker(key, key, {
            [key]: true
          }, null, null, null, runtimeSettings));
        });
      }
      ...
      if (settings.hasOwnProperty("valueOf")) {
        ...
        Object.keys(value).forEach(key => {
          settings.valueOf._.forEach(checker => checker(key, value[key], value, key, value, collection, runtimeSettings));
        });
      }
      ...
      if (settings.shape) {
        Object.keys(settings.shape).forEach(subproperty => {
          settings.shape[subproperty].type._.forEach(checker => checker(subproperty, value[subproperty], value, settings.shape[subproperty].field, document[field], collection, runtimeSettings));
        });
      } else {
    ...
    array: (settings = {}) => generateOptions(previousChecks.concat([(property, data, config, field, document, collection, runtimeSettings) => {
      ...
      if (settings.onCopies) {
        ...
        let isUniqueFunc = settings.onCopies.isUniqueFunc || ((value, array) => {
      ...
      if (settings.of) {
        data.forEach((child, index) => {
          settings.of._[0](`${property}[${index}]`, child, {
            [`${property}[${index}]`]: true
          }, index, document[field], collection, runtimeSettings);
        });
      } else {
        document[field] = data;
      }
...
let docMaker = async (config, configSettings, collection, runtimeSettings) => {
  ...
  if (!configSettings) throw new Error("No configSettings given"); // if (!collection) throw new Error("No collection given");
  ...
  let promises = Object.keys(configSettings).map(property => {
    if (property == "__") {
      return Promise.all(configSettings[property]._.map(checker => checker(null, null, config, null, document, collection, runtimeSettings)));
    }
    ...
    return Promise.all(configSettings[property].type._.map(checker => checker(property, config[property], config, configSettings[property].field, document, collection, runtimeSettings)));
  });
