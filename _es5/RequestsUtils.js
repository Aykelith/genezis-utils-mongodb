...
export const CollectionGenezisConfig = GenezisChecker.or([GenezisChecker.instanceOf(MongoDBCollection), GenezisChecker.function()]);
export const MessageGenezisConfig = GenezisChecker.or([GenezisChecker.string(), GenezisChecker.function()]);
/**
  ...
  of: GenezisChecker.array({
    of: GenezisChecker.object({
      shape: {
        ...
        ___: GenezisChecker.onlyOneAvailable(["input", "constValue"], {
...
export const SingleGetterConfig = { ...getBaseGenezisConfig(),
  ...
  return createRequest(settings, async (req, data, onSuccess, sharedData) => {
    ...
    if (settings.userProjectionAllowed && data[settings.userProjectionInputField]) {
      ...
      if (!Array.isArray(data[settings.userProjectionInputField])) {
        await settings.onError(new GenezisCheckerError(GenezisCheckerErrorTypes.NOT_ARRAY, settings.userProjectionInputField), req, data, sharedData);
      }
    ...
    if (settings.customFindQueryMaker) {
      ...
      if (!findOneQuery) {
        await settings.onError(new GenezisCheckerError(Errors.QUERY_FROM_GIVEN_FIELDS_NOT_FOUND_MATCH), req, data, sharedData);
      }
    ...
    try {
      ...
      if (settings.customOnSuccess) await settings.customOnSuccess(req, data, sharedData, onSuccess, doc);else await onSuccess(doc);
    } catch (error) {
...
export const MultipleGetterConfig = { ...getBaseGenezisConfig(),
...
export const SingleSetterConfig = { ...getBaseGenezisConfig(),
...
export function createSingleSetter(settings) {
  ...
  return createRequest(settings, async (req, data, onSuccess, sharedData) => {
    if (!data[settings.modifiedFieldName]) await settings.onError(new GenezisCheckerError(Errors.EDIT_REQUEST__NO_USER_MODIFIED_ENTRY, settings.modifiedFieldName), req, data, sharedData);
    let findIsEmpty = !data[settings.findFieldName];
    if (!settings.acceptEmptyUserInput && findIsEmpty) await settings.onError(new GenezisCheckerError(Errors.EDIT_REQUEST__NO_USER_FIND_ENTRY, settings.findFieldName), req, data, sharedData);
    ...
    if (settings.updateBy) {
      ...
      if (!sharedData.updateQuery) await settings.onError(new GenezisCheckerError(Errors.QUERY_FROM_GIVEN_FIELDS_NOT_FOUND_MATCH), req, data, sharedData);
    }
    ...
    if (result.modifiedCount != 1) await settings.onError(new GenezisCheckerError(Errors.NO_MODIFIED_DOC), req, data, sharedData);
...
export const SingleAdderConfig = { ...getBaseGenezisConfig(),
  checker: GenezisChecker.required().function(),
...
export function createSingleAdder(settings) {
  ...
  return createRequest(settings, async (req, data, onSuccess, sharedData) => {
    if (!data) await settings.onError(new GenezisCheckerError(Errors.ADD_REQUEST__NO_USER_ADD_ENTRY), req, data, sharedData);
    let doc;
    ...
    if (result.insertedCount != 1) await settings.onError(new GenezisCheckerError(Errors.NO_MODIFIED_DOC), req, data, sharedData);
    ...
    if (settings.customReturn) {
      ...
      await onSuccess(settings.returnTheNewDoc ? result.ops[0] : settings.returnDocField ? result.ops[0][settings.returnDocField].toString() : {});
    }
  });
}
export const SingleDeleterConfig = { ...getBaseGenezisConfig(),
...
export function createSingleDeleter(settings) {
  ...
  return createRequest(settings, async (req, data, onSuccess, sharedData) => {
    ...
    if (settings.oneField) {
      if (!data[settings.oneField.inputFieldName]) await settings.onError(new GenezisCheckerError(Errors.DELETE_REQUEST__NO_INPUT_FIELD_NAME), req, data, sharedData);

      if (settings.oneField.fieldTransformer) {
        data[settings.oneField.inputFieldName] = await settings.oneField.fieldTransformer(data[settings.oneField.inputFieldName], req);
      }
    }
